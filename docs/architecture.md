...
## 3. 사용자 입력 처리 (CJK 한글 지원)

`chat_loop.py`의 `read_line()` 함수. tmux pane 안에서 한글(CJK) 입력의 백스페이스를 정확히 처리한다.

### 문제

tmux 환경에서 한글 입력 후 백스페이스를 누르면 문자가 완전히 지워지지 않는 버그가 발생했다.

**증상:**
- 한글 한 글자를 지울 때 커서만 1칸 이동하고 글자가 화면에 잔존
- 백스페이스를 여러 번 눌러야 한 글자가 지워지거나, 한 번에 두 글자가 지워지는 불규칙한 동작
- 영어는 정상 동작, 한글만 문제 발생

**원인 분석:**

한글은 UTF-8에서 3바이트, 터미널에서 2칸(double-width)을 차지한다. 이 두 가지 특성이 여러 레이어에서 충돌한다:

| 레이어 | 문제 |
|--------|------|
| GNU readline (`input()`) | CJK 문자 폭을 1칸으로 계산 → 커서 위치 오류 |
| 커널 tty 드라이버 (`sys.stdin.readline()`) | `IUTF8` 미설정 시 3바이트 중 1바이트만 삭제 |
| tmux 화면 버퍼 | 커널이 보내는 erase 시퀀스를 wide char에 맞게 처리하지 못함 |

### 시도한 해결 방법

| 시도 | 방식 | 결과 |
|------|------|------|
| 1차 | `input()` + ANSI readline 마커 (`\001`/`\002`) | 실패 — readline의 CJK 폭 계산 자체가 잘못됨 |
| 2차 | `sys.stdout.write()` + `sys.stdin.readline()` | 실패 — 커널 tty 드라이버도 tmux에서 한글 처리 불량 |
| 3차 | `stty iutf8` 플래그 설정 | 실패 — tmux 자체 화면 버퍼가 wide char erase를 올바르게 반영 못함 |
| 4차 | **cbreak 모드 + 수동 폭 계산** | **성공** |

### 현재 구현: `read_line()`

tmux와 커널 tty 드라이버를 모두 우회하고, Python이 직접 문자 단위로 입력을 읽고 화면을 제어한다.

```python
def read_line(prompt="You > ") -> str:
    # 1. tty.setcbreak(fd) — 커널 라인 편집 비활성화, 문자 단위 읽기
    # 2. _read_utf8_char(fd) — UTF-8 바이트를 완전한 한 글자 단위로 조립
    # 3. pos(커서 위치)를 별도 관리 — 중간 삽입/삭제 지원
    # 4. 백스페이스: buf.pop(pos-1) → _redraw_tail()로 뒷부분 재그리기
    # 5. 방향키: ESC[C/D → pos ±1, 폭만큼 커서 이동
    # 6. 중간 삽입: buf.insert(pos, ch) → 뒷부분 재그리기
    # 7. _char_width(ch)로 CJK 2칸 폭 정확히 계산
```

**핵심 함수:**

- `_read_utf8_char(fd)`: 첫 바이트의 상위 비트로 UTF-8 문자의 총 바이트 수를 판단하고, 해당 바이트를 모두 읽어 완전한 문자를 반환
- `_char_width(ch)`: `unicodedata.east_asian_width()`로 문자의 터미널 표시 폭 계산 (W/F → 2칸, 나머지 → 1칸)

**지원하는 입력:**

| 키 | 동작 |
|----|------|
| 일반 문자 (영어/한글) | 커서 위치에 삽입 + 화면 출력 |
| ← → 방향키 | 커서 좌우 이동 (CJK 2칸 폭 반영) |
| Home / End (ESC[H, ESC[F) | 줄 처음/끝으로 이동 |
| Delete (ESC[3~) | 커서 위치 문자 삭제 |
| Backspace/DEL | 커서 앞 문자 삭제 (폭에 맞게) |
| Enter | 입력 완료, 줄 반환 |
...
`termios`/`tty` 모듈은 Unix 전용이므로, Windows에서 실행될 때는 `input()`으로 자동 폴백한다. 실제 runtime은 항상 WSL/tmux이므로 cbreak 모드가 사용된다.

## 4. 작업 종료 감지 (Idle Detection)
...
```
@claude 이 주제 분석해줘      → Claude에만 전송
@claude @codex 비교해줘       → Claude + Codex에 전송
@all 의견 말해봐              → 모든 AI에 전송
(mention 없음)                → 전체 AI에 브로드캐스트
```

@mention이 없으면 모든 활성 AI에게 메시지가 브로드캐스트된다.

### 멀티라인 입력

`"""` 입력으로 멀티라인 모드에 진입하고, 다시 `"""`으로 제출한다.

## 6. /task 오케스트레이션 파이프라인
...
Claude가 모든 계획안을 종합하여 `[model] instruction` 형식으로 작업을 배정한다. 사용자가 확인(`y/n`) 후 진행된다.

`_parse_assignments()`는 `[model] instruction` 패턴을 정규식으로 파싱하며, 멀티라인 지시도 지원한다. 파싱 실패 시 모든 모델에 동일한 지시를 할당하는 폴백이 있다.

### Phase 2: Todo

배정 결과로 `shared/<session>/todo.md` 체크리스트를 자동 생성한다.
...
`_broadcast_result_to_panes()`는 완료된 작업의 합성 결과(최대 2000자)를 모든 AI의 대화형 pane에 전송한다. 메시지 형식:

```
[Previous Task Result] Topic: {task} . Synthesis: {result} .
Remember this context and continue from here.
Wait for the operator's next instruction.
```

이를 통해 `/batch`나 `/task` 완료 후 사용자가 맥락을 다시 설명할 필요 없이 바로 후속 대화를 이어갈 수 있다. 예를 들어 논문 리뷰에서 `/batch 첫 문단 수정` 완료 후 "다음 문단 수정해"라고만 입력하면 AI들이 이전 결과를 이미 알고 있으므로 바로 이어서 작업한다.

결과는 `ConversationLog`에도 기록되어 `/history`로 조회 가능하다.
...
토론 완료 후 합성 결과(최대 2000자)를 모든 AI의 대화형 pane에 브로드캐스트한다. AI들은 이전 토론 맥락을 기억하므로 사용자가 후속 작업을 바로 지시할 수 있다. 결과는 `ConversationLog`에도 기록된다.
...
- **주의**: `--yolo` 모드에서는 Gemini가 자율적으로 파일 읽기/쓰기 등 도구를 실행한다. 팀 컨텍스트에 "사용자가 지시할 때까지 행동하지 마라"는 지시가 포함되어 있으나, 작업 디렉토리에 프로젝트 파일이 있으면 자율 행동을 시작할 수 있다.
